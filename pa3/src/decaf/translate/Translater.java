...
public class Translater {
	...
	public Temp genMOMO(Temp src1, Temp src2, Tree.Expr left) {
		...
		if (left.type.isClassType()) {
			...
			int sz = ((ClassType)left.type).getSymbol().getSize() / OffsetCounter.WORD_SIZE - 1;
			while (sz > 0) {
				Temp t34 = genLoad(src1, OffsetCounter.WORD_SIZE * sz);
				genStore(t34, t33, OffsetCounter.WORD_SIZE * sz);
				sz--;
			}
			genStore(t33, t18, 0);
		}
	...
	public Temp genNewArray(Temp length) {
		genCheckNewArraySize(length, false);
		Temp unit = genLoadImm4(OffsetCounter.WORD_SIZE);
